...
class Registo extends Component {
    ...
    render() {
        ...
        return(
            ...
            <header className="text-white" style={{height: 100 + '%', minHeight: 665 + 'px'}}>
                <div className="container my-auto text-center d-flex" id="hero_title_container" style={{height: '100%', width: '100vw', minHeight: '665px', maxWidth: '100%'}}>
                    {/*style="height: 100%; max-width: 100%"*/}
                    {/* Image Map Generated by http://www.image-map.net/ */}
                    <section style={{zIndex: 0, position: 'absolute', left: 0, width: 100 + '%', height: 100 + '%', minHeight: 665 + 'px',backgroundColor: '#2d142c'}}>
                        <canvas className="canvas" id="myCanvas" style={{height: 100 + '%', minHeight: '600px !important'}} />
                        <div id="particle_count" className="particle-count" />
                        <div id="container_item" className="container-item">
                        </div>
                        <a href="#" id="reset" className="reset-button" />
                    </section>
                    <div className="align-middle" style={{margin: 'auto'}}>
                        <p><img src={logo} style={{}} className="img-fluid img-logo"/></p>
                        <a className="btn js-scroll-trigger mt-4 mx-auto botao_login" style={{zIndex: 4, backgroungImage:'linear-gradient(-120deg, rgb(42,18,46), rgb(156,39,65))', color: 'white'}} href="#login">LOGIN</a>
                    </div>
                </div>
...
function bolhas() {
    ...
    var particleCountDiv = document.getElementById("particle_count");
    ...
    function createCircle(i) {
        ...
        var clicked = false;
        var colour = "rgba(128, 19, 54, 0.8)";
        switch (i) {
            case 1:
                colour = "rgba(128, 19, 54, 0.8)";
                break;
            case 2:
                colour = "rgba(128, 19, 54, 0.8)";
                break;
    ...
    function bubbleClick(event) {
        ...
        for (var i = 0; i < circles.length; i++) {
            if (Math.sqrt((posX - circles[i][0]) * (posX - circles[i][0]) + (posY - circles[i][1]) * (posY - circles[i][1])) < circles[i][2]) {
                if (!circles[i][4]) {
                    circles[i][4] = true;
                    particleCount--;
                }
        ...
        //             }
        //             if (tries === 2) {
        //                 var phraseTextNode = document.createTextNode("-Second try? That's serious");
        //             }
        //             if (tries === 3) {
    ...
    function resetBubbles() {
        ...
        // particleCountDiv.innerHTML = "-bubble count: " + particleCount;
    ...
    function highlightHovered(event) {
        ...
        for (var i = 0; i < circles.length; i++) {
            if (Math.sqrt((posX - circles[i][0]) * (posX - circles[i][0]) + (posY - circles[i][1]) * (posY - circles[i][1])) < circles[i][2]) {
                circles[i][5] = "rgba(128, 19, 54, 0.8)";
                switch (i) {
                    case 1:
                        circles[i][5] = "rgba(128, 19, 54, 0.8)";
                        break;
                ...
                circles[i][5] = "rgba(186, 19, 51, 0.5)";
                switch (i) {
                    case 1:
                        circles[i][5] = "rgba(186, 19, 51, 0.5)";
                        break;
    ...
    function animationLoop() {
        ...
        for (var i = 0; i < circles.length; i++) {
            if (circles[i][0] >
                window.innerWidth + 2 * circles[i][2]
                || circles[i][1] <
                0 - 2 * circles[i][2] || circles[i][2] < 0) {
                if (circles[i][6] === true) {
